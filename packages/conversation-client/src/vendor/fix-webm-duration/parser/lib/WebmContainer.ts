...
type TypeBySectionKey<IdType extends SectionKey> =
  SectionsMap[IdType]["type"] extends keyof SectionTypeMap
...
export class WebmContainer extends WebmBase<WebmContainerItem[], WebmContainerItem[]> {
  ...
  writeUint(x: number, draft = false) {
    let bytes = 1;
    // oxlint-disable-next-line eslint/no-unmodified-loop-condition -- bytes changes the bound
    while ((x < 0 || x >= 2 ** (7 * bytes)) && bytes < 8) bytes++;
    ...
    if (!draft) {
      ...
      this.source![this.offset] = (firstByte & ((1 << (8 - bytes)) - 1)) | (1 << (8 - bytes));
    }
