...
import { err, ok, tryCatch } from "@ai-oral-exam/result";
...
async function createConversation(
  ...
  if (!initialized.value.ok) return err(internalError(initialized.value.error));
...
async function startConversation(
  ...
  if (!applied.value.ok) return err(internalError(applied.value.error));
...
async function conversationState(
  ...
  return stored.value.ok ? ok(stored.value.value) : err(internalError(stored.value.error));
}
